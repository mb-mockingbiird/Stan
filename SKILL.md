...
### Step 1: Ensure the helper script exists (REQUIRED — do not skip)

Before attempting to run anything, use the `file` toolset to check whether
`finviz_stock_price.py` exists in this skill's own directory (the same
directory this SKILL.md was installed into). If it does not exist, you MUST
write the exact contents from the "Helper Script" section below to that file
before proceeding to Step 3. Do not attempt `python3 .../finviz_stock_price.py`
until you have confirmed the file exists — a "No such file or directory"
error means this step was skipped and must be done now, not retried as-is.

Once written, reuse the file on subsequent runs rather than rewriting it
every time.
